...
public class AngleActivity extends AppCompatActivity {
    ...
    private void setupListeners() {
        ...
        // Long press on hypotenuse label to change color
        angleView.setOnHypotenuseLongPressListener(() -> showColorPickerDialog());
    }

    private void loadHypotenuseColor() {
        int savedColor = prefs.getInt(PREF_HYP_COLOR, Color.parseColor("#FFD700"));
        angleView.setHypotenuseColor(savedColor);
    }

    private void saveHypotenuseColor(int color) {
        prefs.edit().putInt(PREF_HYP_COLOR, color).apply();
    }
     ...
     * - n < 10: 2 знака (1:3.44)
     * - 10 <= n < 100: 1 знак (1:10.0, 1:52.1)
     * - n >= 100: без знаков (1:100, 1:542)
     */
    private String formatTaper(double tan) {
        if (tan < 0.001) {
            ...
            double n = 1.0 / tan;
            // Сначала округляем до 2 знаков чтобы проверить границу
            double nRounded2 = Math.round(n * 100.0) / 100.0;
            
            if (nRounded2 < 10) {
                return String.format(Locale.US, "1:%.2f", n);
            } else if (nRounded2 < 100) {
